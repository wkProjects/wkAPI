...
class Webkicks{
    ...
    function getDetails($nick, $adminname=false, $sid=false){
        ...
        $dbview = $this->getcontents("http://server$server.webkicks.de/cgi-bin/tool.cgi?cid=$cid&user=$adminname&pass=$sid&job=search&chatter=$nick");
        //Alias
        preg_match_all('%^\s+<td width="210">(.+?)</td>$%sim',$dbview,$rawdata);
  	preg_match_all('%^\s*<td valign="bottom">G&auml;stebucheintr&auml;ge:</td>\s*<td>(\d+) (?:\((\d+)(?:.+?)\))?%sim', $dbview, $gbdata);
        if (!isset($rawdata[1][15])){ //User ist Hauptadmin!
           $rawdata[1][15]=$rawdata[1][14];
            $rawdata[1][14]=$this->getChatanmeldung();
            $rawdata[1][6]="Hauptadministrator";
        }     
        $data["name"]=$rawdata[1][0];
        $data["alias"]=$rawdata[1][1];
        $data["mail"]=$rawdata[1][2];
		$data["sex"]=$rawdata[1][3];
		$data["home"]=$rawdata[1][4];
        $hplink=preg_replace("%\s*<input class=\"input\" name=\"data2\" size=\"45\" maxlength=\"100\" type=\"text\" value=\"(.+?)\" />\s*%i","$1",$rawdata[1][5]);
        $data["hp"]=($hplink!="http://")?$hplink:"";
		$data["level"]=$rawdata[1][6];
        $data["status"]=$rawdata[1][10];
        $data["ip"]=preg_replace("/ \(.+\)/","",$rawdata[1][13]);
        $data["registered"]=$rawdata[1][14];
        $data["lastlogin"]=$rawdata[1][15];
        $data["uin"]=preg_replace("/\s*<input class=\"input\" name=\"data5\" size=\"45\" maxlength=\"20\" type=\"text\" value=\"(\d*)\" \/>\s*/","$1",$rawdata[1][9]);
        $data["logins"]=preg_replace("/\s*<input class=\"input\" name=\"data4\" size=\"45\" maxlength=\"5\" type=\"text\" value=\"(\d*)\" \/>\s*/","$1",$rawdata[1][8]);
        $data["loginm"]=preg_replace("/\s*<input class=\"input\" name=\"data6\" size=\"45\" maxlength=\"200\" type=\"text\" value=\"([^\"]*)\" \/>\s*/","$1",$rawdata[1][11]);
        $data["logoutm"]=preg_replace("/\s*<input class=\"input\" name=\"data7\" size=\"45\" maxlength=\"200\" type=\"text\" value=\"([^\"]*)\" \/>\s*/","$1",$rawdata[1][12]);
        $data["profil"]=(preg_match("/deaktiviert/i",$rawdata[1][17]))?"deaktiviert":"http://server".$this->server.".webkicks.de/".$this->cid."/pv/".$rawdata[1][0]."/";
        $data["newsletter"]=strip_tags($rawdata[1][16]);
		$data["gbentries"] = (int) $gbdata[1][0];
		$data["points"] = (int) $gbdata[1][1];
        $this->Details[$nick] = $data;
        return $this->Details[$nick];
    }
    ...
    function getReplacers(){
        ...
        $cid = $this->cid;
        $server = intval($this->server);
        $rlist = $this->getcontents("http://server$server.webkicks.de/$cid/rl");
        preg_match_all("%<tr>\s+<td><b><span onclick=\"javascript:repClick\('[^']+'\)\" style=\"cursor:pointer;\">(:.+)</span></b></td>\s+<td><span onclick=\"javascript:repClick\('[^']+'\)\" style=\"cursor:pointer;\"><img src=\"(/$cid/replacer/.+)\"></span></td>\s+</tr>%i",$rlist,$smilies);
        $replacers=array();
        $count = count($smilies[1]);
        if($count!=0){
            for($i=0;$i<=$count;$i++){
                $replacers[$smilies[1][$i]]=$smilies[2][$i];
            }
            array_pop($replacers);
        }
